...
func (handler *Handler) HandlerRefreshToken(w http.ResponseWriter, r *http.Request) {
	...
	// generate new tokens
	tr, newTokenHash, err := auth.GenerateToken(existing.UserID, handler.ApiConfig.JWTSecretKey)
	if err != nil {
	...
	newToken, err := handler.ApiConfig.DB.CreateRefreshToken(r.Context(), database.CreateRefreshTokenParams{
	...
	err = handler.ApiConfig.DB.RotateRefreshToken(r.Context(), database.RotateRefreshTokenParams{
		ID:                existing.ID,
